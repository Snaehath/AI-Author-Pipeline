...
class NormalizedTextDeduplicator:
    ...
    def is_duplicate(self, text: str, source_id: str) -> Tuple[bool, Optional[str]]:
        ...
        # 2. Check near-duplicate via Jaccard overlap on 3-grams
        words = norm.split()
        if len(words) < 15:
        ...
        for canonical_id, prev_words, prev_ngrams in self._registered_passages:
    ...
    def register(self, text: str, source_id: str) -> None:
        ...
        words = norm.split()
        ngrams = self._compute_word_ngrams(words, n=3)
        self._registered_passages.append((source_id, set(words), ngrams))
